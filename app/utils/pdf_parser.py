...
def extrair_dados_nota_corretagem(caminho_arquivo):
    try:
        ...
        # 1. Busca o Líquido (Total líquido da nota)
        v_liquido = 0.0
        match_liquido = re.findall(r"(\d[\d\.,]+\d{2})\s*$", texto_completo, re.MULTILINE)
        if match_liquido:
            v_liquido = limpar_valor(match_liquido[-1])

        # 2. Busca IRRF (1%) e Taxas B3 (BM&F + Registro)
        # Padrão Genial: Projeção 1% | D | 0,00 | Taxa 1 | Taxa 2
        match_resumo = re.search(r"IRRF Day Trade.*?([\d\.,]+\d{2})\s+D\s+0,00\s+([\d\.,]+\d{2})\s+([\d\.,]+\d{2})", texto_completo, re.DOTALL)
        
        v_irrf = 0.0
        v_taxas = 0.0
        if match_resumo:
            v_irrf = limpar_valor(match_resumo.group(1)) # 14,83
            v_taxas = limpar_valor(match_resumo.group(2)) + limpar_valor(match_resumo.group(3)) # 56,32 + 31,68

        # 3. Cálculo do Bruto Real (Líquido + IRRF + Taxas)
